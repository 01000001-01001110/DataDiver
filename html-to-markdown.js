...
async function convertToMarkdown(source, outputPath) {
    try {
...
async function extractImages(source, outputPath) {
    try {
...
if (process.argv.length > 2) {
    const source = process.argv[2];  // Can be URL or file path
    const outputFile = process.argv[3] || 'output.md';
    
    if (process.argv.includes('--images')) {
        extractImages(source, outputFile);
    } else {
        convertToMarkdown(source, outputFile);
    }
}

export { convertToMarkdown, extractImages };
